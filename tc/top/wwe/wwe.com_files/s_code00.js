...
var s_account="wweconsumer"
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
s.dynamicAccountSelection=true
s.dynamicAccountList="devwweconsumer=stage.wwe.com,stage1.wwe.com,stage2.wwe.com,stage3.wwe.com,dev1.wwe.com,dev2.wwe.com,dev3.wwe.com,qa1.wwe.com,qa2.wwe.com,qa3.wwe.com,proxy.wwe.com,proxy2.wwe.com,proxy3.wwe.com,localhost"
s.charSet="UTF-8"
s.currencyCode="USD"
...
s.linkInternalFilters="javascript:,wwe.com,localhost,kable.com/pub/wwet/,wwegame.kp9.net,wwe.kp9.net,biggestfan.kp9.net,wwe.channelfinder.net,coolsavings.com"
...
function s_doPlugins(s) {
	if (s.pageType != 'errorPage') {
		/* Page URL */
		s.prop1 = window.location.href.substr(0,99);

		/* WWE Account ID */
		if ((typeof(visitordata) != 'undefined') && (typeof(visitordata.s) != 'undefined')) {
			s.prop31 = visitordata.s;
			s.eVar31 = s.prop31;
		}
		...
		if (typeof(s.events) === "undefined") {
			s.events = "event7";
		} else if (s.events.indexOf("event7") === -1) {
			s.events += ",event7";
		}

		/* Campaign Variables */
		s.campaign = s.getValOnce(s.getQueryParam('cid'), 'cid');
		s.eVar37 = s.getValOnce(s.getQueryParam('iid'), 'iid');
		s.eVar38 = s.getValOnce(s.getQueryParam('hid'), 'hid');
		s.eVar39 = s.getValOnce(s.getQueryParam('sid'), 'sid');

		/* Search Term */
		s.prop6 = s.getQueryParam('q').toLowerCase();
		s.eVar1 = s.prop6;

		/* New vs. Repeat Visitor */
		s.eVar40 = s.getValOnce(s.getNewRepeat(),'t_nr');
		
		/* Silverlight Plug-in Version (once per-session) */
		if ((typeof(Silverlight) != 'undefined') && (typeof(sessionData) != 'undefined')) {
			if (!sessionData.silverlightVersion) {
				sessionData.silverlightVersion = (Silverlight.isInstalled('1.0') ? (Silverlight.isInstalled('1.1') ? '1.1' : '1.0') : 'Not Installed');
				sessionData.store();
			}
			s.prop20 = sessionData.silverlightVersion;
		}
		
		/* Flash Plug-in Version */
		if (typeof(deconcept) != 'undefined') {
			var version = deconcept.SWFObjectUtil.getPlayerVersion();
			s.prop23 = (version["major"] > 0 ? version['major'] + '.' + version['minor'] + '.' + version['rev'] : 'Not Installed');
		}
...
+"s='wwe.com_files/s11437877735588.gif'`6^M$B^B`R`H>5.5)rs=^Urs,4095);`r rs=^Urs,2047)}^1s.d"
+".images&&`H>=3^W!s.^R||`H>=7)^W@9<0||`H>=6.1)`R!s.rc)s.rc`C`6!^F){^"
+"F=1`6!s.rl)s.rl`C;^jn]`Y@2;set@Hout('`k,750)}`r{l=^jn]`6l){r.t=ta;r"
+".u=un;r.r=wwe.com_files/s11437877735588.gif;l[l`B]=r;`2''}imn+='^t^F;^F$Aim=`E[imn]`6!im)im=`E[imn"
+"]`YImage;im@6l=0;im.@G`YF`K('e`z^u@6l=1;`k);im$P=rs`6rs`1@k=@N0^W!t"
+"a||ta`g_self@wa`g_top'||(`E.^f@Oa==`E.^f))){b=e`Y^e;`P!im@6l&&e^K-b"
+"^K<500)e`Y^e}`2''}`2'<im'+'g sr'+'c=\"wwe.com_files/s11437877735588.gif\" width=1 $H=1 border="
