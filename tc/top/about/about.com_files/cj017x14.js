...
q='>';zg=zCn=zSbL=zAc=zGARF=zDMN=zPxzTL=0;z0=zpT=zast=zChA=zAds='';d=document;zDL=new Date();zOr=(this.zOrO?(zr(99999)+zr(99999)):zOr)+zr(99999);zAUC=zBb=zBbo=zBbS0=zTbC=0;zBbS1=1
...
if(zAC)zc(1,'zFS=',zFS,99999999,gs);if(this.zAth&&zAth.length>0)zPxT('a='+zAth+'&u='+zWl)
if(this.zFD&&(zFD>zc(0,'zFD=')))zc(1,'zFD=',zFD,99999999,'')
zRA=new Array('google','yahoo','aol','msn','teoma','alltheweb','ask','looksmart','excite','dogpile','mamma','netscape','microsoft','my.yahoo')
zR=zRef=document.referrer;zR=zrp(zR,zh,'');zR=zrp(zR,'https://','');i=zR.indexOf('/');if(i>0)zR=zR.substring(0,i)
zRf=zc(4,'zRf');if(zwl.href.indexOf("?nl=1")>0){zc(5,'nl',1,44444,gs);if(zRf=='')zRf=-3}
if(zRf==''){zRf=(zR.indexOf(uy)>-1)?0:(zR==''?-2:-1);for(var i=0;i<zRA.length;)if(zR.indexOf(zRA[i++]+'.')>-1)zRf=i}
zc(5,'zRf',zRf,30,'')
...
if(this.zGRCn){}else
google_ad_channel=(this.zGRC&&zGRC.length?zGRC:('primedia_'+gs+((zG&8)&&this.zGOP?'_offers':'')))+(this.zGCID?zGCID:'')
google_ad_output='js'
...
mB=this.zD336&&(g==336)?3:1;ax='/'+bp+((td==uy)&&c.length?'abt.'+c:td)+'/'+c+'_'+gs+';'+zap+'svc='+s+';site='+(zgs.length?zgs:gs)+';t='+zTa+';bt='+zBT+';bts='+zBTS+((zDot==3)&&zbDM?zDMD():'')+';pc='+zpc+';auc='+(++zAUC)+';fd='+zFDT+';fs='+zFST+(this.zir?';r='+zir:'')+';sp2='+(zbXP2?1:0)+';gr='+escape(zWl)+(zs>0?';go='+zGSk:'')+';a='+zc(0,'aut=')+tm+';kw='+k+';chan='+c+';syn=about;tile='+p+(mB==3?';af=0':'')+';r='+zRf+zTc()+(bT?';dcopt=ist':'')+';sz='+d+';ord='+(bH?'1':o)+zOr
...
if(m&2){zAD.w(n);zAD.w(x)}w('<iframe id="'+n+'" name="'+n+'" '+(f&24576?'onresize="zCh(\''+n+'\','+f+')" onload="zCh(\''+n+'\','+f+')" ':'')+' src="about.com_files/ad.html" marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no'+hw+qz+'</iframe>');}
...
s=zc(0,'zsp=');if((s||(ztp&&(ztp>zr(100))))&&ztp>-1){l=document.URL;w('<img src="about.com_files/default.gif">');ztp=-1}
if(parseInt(zbV)<4||zTV)return
if(1==zr(100))zau(256,1,1,'px3',zh+'z.'+uy+'/px/?p=ggl'+zs+(zG&16?'p':'b')+zTa+'_'+(this.zLb?zLb:0)+(this.zGCID?'&ggt='+zE(zGCID):'')+'&w='+zIfw+'&gs='+gs+'&tt='+zTa+'&bt='+zBT+'&bts='+zBTS+'&bbaf='+z+'&a='+(zAC?1:0)+zSm+(zSu.length?'&su='+zSu:''),'')}
...
function zPxT(q){zPxA[zPxC]=new Image();zPxA[zPxC++].src="about.com_files/default.gif"}
function google_radlink_request_done(a){zOfs=this.zAS?zCpy(a):a;zOfsL=a.length;zGARD()}
function zGARD(){var a=this.zRad,b=a?(zOfsL>=0):1;if(b&&(zs>=0)&&(zGARF<1)){zGARF=1;if(100>zr(1000))zPxT('gard=1&gs='+gs+'&c='+zs+(a?'&r='+zOfsL:'')+'&tt='+zTa+'&bt='+zBT+'&bts='+zBTS)
}}
...
while((i<l)&&i<zRad){t=a[i++].line1;w('<a href="/z/js/o'+(p?p:'')+'.htm?k='+escape(t.toLowerCase())+'&d='+escape(t)+'&r='+escape(zWl)+'">'+t+'</a>')}
...
function zMu(l,p,e,x){var m=new Date(),n=m.getTime()-zDL.getTime(),u='_',t=l.href;return zh+'clk.'+uy+'/?zi='+p+(l.name?'&zn='+l.name:'')+(x?'&zTi=1':'')+(this.zi?'&sdn='+gs+'&cdn='+ch:'')+'&tm='+Math.round(n/1000)+(zTbC?'&acs='+zTbC:'')+(zbI?'&gps='+e.offsetX+u+e.offsetY+u+zIfw+u+zIfh:'')+'&f='+zFDT+zFST+zSm+(zSu.length?'&su='+zSu:'')+'&tt='+zTa+'&bt='+zBT+'&bts='+zBTS+'&zu='+zrp(zE(l.href),'+','%2b')}
...
if(s){z=s.getAttribute("zT");if(z){zPxzT(zMu(s,z,window.event,1),35);return r}
...
if(s.lnp)zPxT('glnp='+s.lnp+'&gs='+gs+'&t='+zTa+'&bt='+zBT+'&bts='+zBTS);if(o0){x=t.indexOf('zu=');if(x>-1){t=t.substring(x);o0=t.indexOf(uy)==-1}}
else if(o1){t=t.substring(y+17);o1=t.indexOf(uy)==-1}
if(o0||o1||o2)if(t.indexOf(zh)==0){if(s.target.length<1)s.target=n+zr(999);r=s.f=1}}else r=s.f}};return r}
