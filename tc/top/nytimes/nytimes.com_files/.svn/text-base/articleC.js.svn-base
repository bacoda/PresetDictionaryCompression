/**
* $Id: articleCommentCounts.js,v 1.3 2008/06/02 20:25:48 abehrens Exp $
* 
...
ArticleCommentCounts = (function() {
	...
	function getReadAllCommentsLink(data, node) {
		return (data.count > 0) ? link(getOverflowUrl(node), getReadCommentsLinkText(data) + getCountText(data)) : emptyNode();
	}
	
	function getDivider(data) {
		return (data.commentsEnabled && data.count > 0) ? document.createTextNode(" | ") : emptyNode();
	}
	...
	function getArticleId(node) {
		return node.getAttribute("articleid");
	}
