...
function checkteams(){
	...
	if(nbaSiteVar=="%26nbasite%3Deuropelive2007"){nbaSiteVar = "%26nbasite%3Deuropelive"}
	return nbaSiteVar;
}
checkteams("celtics", "heat", "nets", "knicks", "magic", "sixers", "wizards", "hawks", "bulls", "cavaliers", "pistons", "pacers", "bucks", "hornets", "raptors", "mavericks", "nuggets", "rockets", "grizzlies", "timberwolves", "spurs","jazz", "warriors", "clippers", "lakers", "suns", "blazers", "kings", "sonics", "espanol", "france", "uk", "brasil", "germany", "japan","canada","bobcats","europelive2007")

function nbaVideo(src,video){
if(video=="team"){
src = src + "%26lc1%3DNBA.com%26lc2%3Dteamsites%26siteid=team";
}
if(src.indexOf("nbacom/tnt/yellowbook")>0||src.indexOf("nbacom/tnt/inside")>0||src.indexOf("nbacom/tnt/halftime")>0){
	src = src + "%26nbasite%3D"  + "yellowbook"
}
if(src.indexOf("mvp")>0){
	src = src + "%26nbasite%3D"  + "kia"
}
if(src.indexOf("nbacom/draft/profiles")>0){
	src = src + "%26nbasite%3D"  + "profiles"
}
...
tokenPath = "/broadband/Connection.jsp?dest=" + src
		window.open(tokenPath,"BroadbandWin", "width=765,height=480,top=100,left=200,alwaysRaised=yes,toolbar=0,directories=0,menubar=0,status=0,resizable=no,location=0,scrollbars=1,copyhistory=0");
}
...
var gopen_window_ex = open_window_ex;

function open_window_ex_link(url, title, options) {
	mywin=window.open(url,title,options);
}
function reFocusALPWindow(){
	var spawn = window.open("/broadband/alp_schedule.html", "AudioLeaguePass","toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=0,width=660,height=550");
	spawn.focus();
	document.location='http://www.nba.com' ;
	}
...
function nbaAudio() {
	open_window_ex('http://www.nba.com/broadband/alp_schedule.html', 'AudioLeaguePass', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=0,width=660,height=550')
}