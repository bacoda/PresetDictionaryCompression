	...
	{ 
		...
		{
			this.obj.style[ stylePropertyName ] = 'rgb('+intval+','+intval+','+intval+')'
		}else 
		if (stylePropertyName == 'opacity')
		{
			this.obj.style[ stylePropertyName ] = intval / 100;
			this.obj.style.filter = 'alpha(opacity=' + intval + ')'
		}
		else{
			//console.log('just setIntegerStyleProperty',intval,Math.round(value),this.unit,stylePropertyName);
			//this.obj.style[ stylePropertyName ] = Math.round( value ) + this.unit 
			this.obj.style[stylePropertyName] = intval+this.unit;
		}
	}
	...
	// {
	//@param t stands fpr
	//@param b stands for start
	//@param c stands for length??? 
	//@param d stands for duration
