/*       Source:  Local Cache                                                                 */
/*     Location:  rsrc:121439:nu_ll:/html/js/common.js.pkg.php:92                             */
/*      Machine:  10.16.139.109                                                               */
/*    Generated:  September 13th 2008 1:22:03 AM PDT                                          */
...
return fn;};function goURI(uri){uri=uri.toString();if(window.PageTransitions){PageTransitions.go(uri);}else if(window.location.href==uri){window.location.reload();}else{window.location.href=uri;}}
...
return obj;}
var Bootloader={loadComponents:function(components,callback){components=arrayize(components);var required_resources=[];for(var ii=0;ii<components.length;++ii){var component_resource_list=this._component_map[components[ii]];if(!component_resource_list){throw new Error("Can not bootload `"+components[ii]+"'.");}
for(var jj=0;jj<component_resource_list.length;++jj){required_resources.push(component_resource_list[jj]);}}
this.loadResources(required_resources,callback);},loadResources:function(resources,callback){resources=arrayize(resources);var request_index=this._pending.length;var pending_request=false;for(var ii=0;ii<resources.length;++ii){var rsrc=resources[ii];if(this._loaded[rsrc.name]){continue;}
if(rsrc.type=='js'&&callback){if(!this._pending[request_index]){this._pending[request_index]={names:{},callback:callback};}
this._pending[request_index].names[rsrc.name]=true;pending_request=true;}
if(!this._requested[rsrc.name]){this.requestResource(rsrc.type,rsrc.src);this.requested(rsrc.name);}}
if(!pending_request&&callback){this._invoke(callback);}},requestResource:function(type,source){var h=this._getHardpoint();switch(type){case'js':var script=document.createElement('script');script.src=source;script.type='text/javascript';h.appendChild(script);break;case'css':var link=document.createElement('link');link.rel="stylesheet";link.type="text/css";link.media="all"
link.href=source;h.appendChild(link);break;default:throw new TypeError("Bad resource type `"+type+"'.");}},done:function(names){this.requested(names);for(var ii=0;ii<names.length;++ii){var loaded_resource=names[ii];this._loaded[loaded_resource]=true;for(var jj=0;jj<this._pending.length;++jj){var required_resources=this._pending[jj].names;delete required_resources[loaded_resource];if(is_empty(required_resources)){var callback=this._pending[jj].callback;this._pending.splice(jj,1);--jj;if(callback){this._invoke(callback);}}}}},requested:function(resources){resources=arrayize(resources);for(var ii=0;ii<resources.length;++ii){this._requested[resources[ii]]=true;}},enableBootload:function(map){copy_properties(this._component_map,map);},_invoke:function(callback){setTimeout(callback,0);},_getHardpoint:function(){if(!this._hardpoint){var n,heads=document.getElementsByTagName('head');if(heads.length){n=heads[0];}else{n=document.body;}
this._hardpoint=n;}
...
element.__listen__=element.__listen__||{};var handlers=element.__listen__;event_name=Event._normalizeName(event_name);Event._attachHandler(element,event_name);var type_handlers=handlers[event_name];if(!(priority in type_handlers)){type_handlers[priority]=[];}
...
return event_name;};Event._attachHandler=function(element,event_name){var full_event_name='on'+event_name;var handlers=element.__listen__;if(event_name in handlers){if(element[full_event_name]!==Event._listenHandler){throw new Error("`"+full_event_name+"' listen handler gone!");}
return;}
...
var type_handlers=this.__listen__[type];if(!type_handlers){throw new Error("No registered handlers for `"+type+"'.");}
...
return result;},implodeQuery:function(obj,name){name=name||'';var r=[];if(obj===null){r.push(URI.encodeComponent(name));}else if(obj instanceof Array){for(var ii=0;ii<obj.length;++ii){try{if(obj[ii]!==undefined){r.push(URI.implodeQuery(obj[ii],name?(name+'['+ii+']'):ii));}}catch(ignored){}}}else if(typeof(obj)=='object'){if(is_node(obj)){r.push('{node}');}else{for(var k in obj){try{r.push(URI.implodeQuery(obj[k],name?(name+'['+k+']'):k));}catch(ignored){}}}}else{r.push(URI.encodeComponent(name)+'='+URI.encodeComponent(obj));}
...
var query=this.getQueryData();for(var i=0,l=keys.length;i<l;++i){delete query[keys[i]];}
...
return(new Vector2(left,top,'document').convertTo(domain));},getElementDimensions:function(el){if(ua.safari()&&el.nodeName=='TR'){var tds=el.getElementsByTagName('td');var dimensions=Vector2.getElementCompassPoint(tds[tds.length-1],Vector2.compass.southeast).sub(Vector2.getElementPosition(tds[0]));return dimensions;}
...
Util.error('Unknown compass point %s.',which);return p;},getViewportDimensions:function(){var x=(window&&window.innerWidth)||(document&&document.documentElement&&document.documentElement.clientWidth)||(document&&document.body&&document.body.clientWidth)||0;var y=(window&&window.innerHeight)||(document&&document.documentElement&&document.documentElement.clientHeight)||(document&&document.body&&document.body.clientHeight)||0;return new Vector2(x,y,'viewport');},getDocumentDimensions:function(){var x=(document&&document.body&&document.body.scrollWidth)||(document&&document.documentElement&&document.documentElement.scrollWidth)||0;var y=(document&&document.body&&document.body.scrollHeight)||(document&&document.documentElement&&document.documentElement.scrollHeight)||0;return new Vector2(x,y,'document');},scrollTo:function(v,use_animation){use_animation=use_animation||use_animation===undefined;if(!(v instanceof Vector2)){var x=Vector2.getScrollPosition().x;var y=Vector2.getElementPosition($(v)).y;y=y-Math.min(0,Math.max(Vector2.getViewportDimensions().y/3,100));v=new Vector2(x,y,'document');}
v=v.convertTo('document');if(use_animation&&window.animation){animation(document.body).to('scrollTop',v.y).ease(animation.ease.end).duration(750).go();}else if(window.scrollTo){window.scrollTo(v.x,v.y);}}});var mouseX=function(e){return Vector2.getEventPosition(e).x;}
...
obj=document.getElementById(id);if(!(ua.ie()>=7)){return aug(obj);}
...
el.appendChild(node);}}else{Util.error('No way to set content %q.',content);}
return el;},remove:function(element){element=$(element);if(element.removeNode){element.removeNode(true);}else{for(var ii=element.childNodes.length-1;ii>=0;--ii){DOM.remove(element.childNodes[ii]);}
...
return element;},createFromMarkup:function(markup){var wrapper_elm=document.createElement('div');set_inner_html(wrapper_elm,markup);return wrapper_elm.childNodes;},find:function(element,selector_str){var elements=DOM.scry(element,selector_str);if(1!=elements.length){Util.error('DOM.find located %d elements that matched the selector `%s\''+'.  Please only used find when you are sure that 1 element '+'should match (like $).  Otherwise, please use DOM.scry.',elements.length,selector_str);return null;}
...
var selector=selectors[i];var new_candidates=[];selector=selector.replace(/\./g,' .');selector=selector.replace(/\#/g,' #');selector=selector.replace(/\[/g,' [');var sub_selectors=selector.split(' ');var tag=sub_selectors[0]||'*';if(sub_selectors[1]&&sub_selectors[1].charAt(0)=='#'){var elem=ge(sub_selectors[1].slice(1));if(elem&&('*'==tag||elem.tagName.toLowerCase()==tag)){for(var iter=0;iter<candidates.length;iter++){if(document==candidates[iter]||is_descendent(elem,candidates[iter])){new_candidates=[elem];break;}}}}else{var tag_candidates=[];var candidate_l=candidates.length;for(var ii=0;ii<candidate_l;ii++){var elements=candidates[ii].getElementsByTagName(tag);var elements_l=elements.length;for(var jj=0;jj<elements_l;jj++){tag_candidates.push(elements[jj]);}}
...
if(!no_augment){var augment=DOM._getNodeAugment();for(var ii=0;ii<candidates.length;ii++){augment(candidates[ii]);}
list=new NodeList();list.push.apply(list,$A(candidates));}else{list=candidates;}
return list;},prependChild:function(parent,child){parent=$(parent);if(parent.firstChild){parent.insertBefore(child,parent.firstChild);}else{parent.appendChild(child);}},insertAfter:function(parent,child,element){if(parent!=child.parentNode){Util.error('child is not really a child of parent - wtf, seriously.');}
if(child.nextSibling){var ret=parent.insertBefore(element,child.nextSibling);}else{var ret=parent.appendChild(element);}
if(!ret){return null;}
...
if(is_node(element,'input')){var range=document.selection.createRange();return{start:-range.moveStart('character',-element.value.length),end:-range.moveEnd('character',-element.value.length)};}else{var range=document.selection.createRange();var range2=range.duplicate();range2.moveToElementText(element);range2.setEndPoint('StartToEnd',range);var end=element.value.length-range2.text.length;range2.setEndPoint('StartToStart',range);return{start:element.value.length-range2.text.length,end:end};}},setCaretPosition:function(obj,start,end){if(document.selection){if(obj.tagName=='TEXTAREA'){var i=obj.value.indexOf("\r",0);while(i!=-1&&i<end){end--;if(i<start){start--;}
...
range.select();}else{obj.selectionStart=start;var sel_end=end==undefined?start:end;obj.selectionEnd=Math.min(sel_end,obj.value.length);obj.focus();}},addEvent:function(element,type,func,name_hash){return addEventBase(element,type,func,name_hash);},_nodeAugment:null,_getNodeAugment:function(){if(!DOM._nodeAugment){var aug=new Augment('NodeAugment',function(){var bind1st=function(fn){return function(){return fn.apply(null,[this].concat($A(arguments)));}}
...
var elements=root_element.getElementsByTagName(this._controlled_tag_name);var event_name=this._controlled_event_name.substring(2);var onevent=this._onevent;var element;try{for(var i=elements.length-1;i>=0;--i){element=elements[i];Event.listen(element,event_name,onevent,Event.Priority.LINK_CONTROLLER);}}catch(ex){Util.error('Uncaught exception while reading %s handler for element with contents %h: %s',event_name,element.innerHTML,ex);}
...
this._positionMenu();if(!this.isShown){this._show();}else{this._hide();}
return false;}
dropmenu.prototype.show=function(){if(!this.menu){this._buildMenu();}
this._positionMenu();this._show();}
...
dropmenu.INSTANCES[this.id]=this;this.menuClickArea=this.menuClickArea||DOM.scry(this.menu,'div.menu_content');this.menuClickArea.onclick=chain(this.menuClickArea.onclick,Event.stop);var menu_links=DOM.scry(this.menu,'a');for(var i=0;i<menu_links.length;i++){addEventBase(menu_links[i],'click',function(){this._hide();}.bind(this),this.menu_id+'_link_'+i);}}
...
dropmenu.prototype._hide=function(){CSS.addClass(this.menu,'hidden_elem');removeEventBase(document.body,'click',this.click_event,this.menu.id);this.click_event=null;this.isShown=false;for(var i=0;i<this.hide_hooks.length;i++){this.hide_hooks[i]();}}
...
function adjustImage(obj,stop_word,max){var block=obj.parentNode;while(CSS.getStyle(block,'display')!='block'&&block.parentNode){block=block.parentNode;}
...
function cavalry_log(cohort,server_time,filepath,pagetype,rate){if(!window.Env||typeof(___t_measuring)=='undefined'||!___t_measuring){___t_measuring=false;return;}
...
var rate_report='';if(rate){rate_report='&rate='+rate;}
...
ua.populate();_bootstrapEventHandlers();adjustUABehaviors();if(navigator&&navigator.userAgent&&document.domain.toLowerCase().indexOf('facebook.com')!=-1&&!(parseInt((/Gecko\/([0-9]+)/.exec(navigator.userAgent)||[]).pop())<=20060508)){document.domain='facebook.com';}
...
function AsyncRequest(){var dispatchResponse=bind(this,function(asyncResponse){try{this.clearStatusIndicator();if(this.initialHandler(asyncResponse)!==false){if(this.handler){try{this.handler(asyncResponse);}catch(exception){this.finallyHandler(asyncResponse);throw exception;}}
...
if(this.handler||this.errorHandler||this.transportErrorHandler){if(typeof(interp.transportError)!='undefined'){var r=new AsyncResponse();var errorDescription=Util.isDevelopmentEnvironment()?interp.transportError:tx('async:error');copy_properties(r,{error:1000,errorSummary:tx('async:oops'),errorDescription:errorDescription});if(this.transportErrorHandler){dispatchErrorResponse(r,true);}else{Util.error('Something bad happened; provide a transport error handler for '+'complete details.');}
return;}
...
fn=fn.shield(null,r);fn=fn.defer.bind(fn);if(r.bootload){Bootloader.loadResources(r.bootload,fn);}else{fn();}}});var invokeErrorHandler=bind(this,function(explicitError){try{if(!window.loaded){return;}}catch(ex){return;}
...
this.transport=transport;try{this.transport.open(this.method,uri,this.getOption('asynchronous'));}catch(ex){Util.error('Exception when opening Async transport to %q: %x',uri,ex);return false;}
...
return false;},filterEventTargets:function(event,type){var target=event_get_target(event);if(is_node(target,['input','select','textarea','object','embed'])){return false;}
...
function ajax_tz_set(tzForm){var timestamp=tzForm.time.value;var gmt_off=-tz_calculate(timestamp);var cur_gmt_off=tzForm.tz_gmt_off.value;if(gmt_off!=cur_gmt_off){var ajaxUrl='www.facebook.com_files/timezone.js';new AsyncSignal(ajaxUrl,{user:tzForm.user.value,post_form_id:tzForm.post_form_id.value,gmt_off:gmt_off}).send();}}
function tz_autoset(){var tz_form=ge('tz_autoset_form');if(tz_form)
ajax_tz_set(tz_form);}

function typeaheadpro(obj,source,properties){if(!typeaheadpro.hacks){typeaheadpro.should_check_missing_events=ua.safari()<500;typeaheadpro.should_simulate_keypress=ua.ie()||(ua.safari()>500&&ua.safari()<523||ua.safari()>=525);if(typeaheadpro.should_use_iframe==undefined){typeaheadpro.should_use_iframe=typeaheadpro.should_simulate_keypress;}
typeaheadpro.should_use_overflow=ua.opera()<9.5||ua.safari()<500;if(ua.firefox()){this.poll_handle=setInterval(this.check_value.bind(this),100);this.deactivate_poll_on_blur=false;}
...
typeaheadpro.prototype._onkeydown=function(e){this.key_down=this.last_key=e?e.keyCode:-1;this.interactive=true;switch(this.last_key){case 33:case 34:case KEYS.UP:case KEYS.DOWN:if(typeaheadpro.should_simulate_keypress){this._onkeypress({keyCode:this.last_key});}
return false;case KEYS.TAB:this.select_suggestion(this.selectedindex);if(e.shiftKey){this.reverse_focus();}else{this.advance_focus();}
break;case KEYS.RETURN:if(this.select_suggestion(this.selectedindex)){this.hide();}
...
typeaheadpro.prototype._onfocus=function(){if(this.source){this.source.bootstrap();}
...
html.push('" onmouseover="typeaheadpro.instances[',instance,'].mouse_set_suggestion(',i,')" ','onmousedown="var instance=typeaheadpro.instances[',instance,']; instance.select_suggestion(',i,');instance.hide();event_abort(event);">',this.source.gen_html(suggestions[i],text),'</div>');}
...
generic_dialog.prototype.focus_first_textbox_or_button=function(){var selectors=['input[type="text"]','textarea','input[type="button"]','input[type="submit"]'];var nodes=[];for(var ii=0;ii<selectors.length&&nodes.length==0;ii++){nodes=DOM.scry(this.content,selectors[ii]);}
...
Dialog.OK={name:'ok',label:tx('sh:ok-button')};Dialog.CANCEL={name:'cancel',label:tx('sh:cancel-button'),className:'inputaux'};Dialog.CLOSE={name:'close',label:tx('sh:close-button')};Dialog.SAVE={name:'save',label:tx('sh:save-button')};Dialog.SUBMIT={name:'submit',label:tx('sh:submit-button')};Dialog.OK_AND_CANCEL=[Dialog.OK,Dialog.CANCEL];Dialog._STANDARD_BUTTONS=[Dialog.OK,Dialog.CANCEL,Dialog.CLOSE,Dialog.SAVE];Dialog.newButton=function(name,label,className){var button={name:name,label:label}
if(className){button.className=className;}
return button;}
Dialog.getCurrent=function(){var stack=generic_dialog.dialog_stack;if(!stack||stack.length==0){return null;}
...
return this;},setShowing:function(){this.show();return this;},setTitle:Dialog._basicMutator('_title'),setBody:Dialog._basicMutator('_body'),setExtraData:Dialog._basicMutator('_extra_data'),setShowLoading:Dialog._basicMutator('_show_loading'),setUserData:Dialog._basicMutator('_user_data'),getUserData:function(){return this._user_data;},setAutohide:function(autohide){if(autohide){if(this._showing){this._autohide_timeout=setTimeout(bind(this,'hide'),autohide);}else{this._autohide=autohide;}}else{this._autohide=null;if(this._autohide_timeout){clearTimeout(this._autohide_timeout);this._autohide_timeout=null;}}
...
this._modal=modal;return this;},setContentWidth:function(width){this._content_width=width;this._dirty();return this;},setClassName:Dialog._basicMutator('_class_name'),setCloseHandler:function(close_handler){this._close_handler=call_or_eval.bind(null,this,close_handler);return this;},setFooter:Dialog._basicMutator('_footer'),setAutoFocus:Dialog._basicMutator('_auto_focus'),onloadRegister:function(handler){this._onload_handlers.push(handler.bind(this));return this;},setAsync:function(async_request){var handler=function(response){if(this._async_request!=async_request){return;}
...
this._pd.set_auto_focus(this._auto_focus===undefined||this._auto_focus);if(!this._async_request||!this._show_loading){var html=[];if(this._title){html.push('<h2><span>'+this._title+'</span></h2>');}
...
var inputs=DOM.scry(this._pd.popup,'input');var name_filter=function(elem){return elem.name==button.name;};return inputs.filter(name_filter)[0]||null;},_submitForm:function(method,uri,button){var data=this._getFormData();data[button.name]=button.label;if(this._extra_data){copy_properties(data,this._extra_data);}
var async_request=new AsyncRequest().setURI(uri).setData(data).setMethod(method).setReadOnly(method=='GET');this.setAsync(async_request);return false;},_getFormData:function(){var dialog_content_divs=DOM.scry(this._pd.content,'div.dialog_content');if(dialog_content_divs.length!=1){Util.error(dialog_content_divs.length
...
Dialog.createCaptchaDialog=function(captchaHtml,closeHandler){var dialog=new Dialog().setClassName('captcha_dialog').setTitle(tx('rec18')).setBody(captchaHtml).setButtons([Dialog.SUBMIT,Dialog.CANCEL]).setCloseHandler(closeHandler).setModal().onloadRegister(create_captcha);return dialog;}

function get_redesign_feedback_js(get_url,post_url){var async=(new AsyncRequest()).setReadOnly(true).setMethod("GET").setURI(get_url);var current_url=window.HistoryManager?HistoryManager.getCurrentFragment():URI.getRequestURI().getPath()+('?'+URI.implodeQuery(URI.getRequestURI().getQueryData()));new Dialog().setStackable(true).setAsync(async).setPostURI(new URI(post_url).addQueryData({url:current_url}).toString()).show();return false;}
function validate_redesign_feedback(){if($('feedback_type').value=='unselected'){new ErrorDialog().showError(tx('rdf:error-title'),tx('rdf:error-body'));return false;}
return true;}
...
function share_show_error(params){var dialog=new Dialog().setTitle(tx('sh01')).setBody(tx('sh02')).setButtons(Dialog.OK).show();return false;}
function share_is_shareable(url){return(/(?:https?:\/\/)?\w+\.\w+/.test(url));}
function share_internal_bookmarklet(){if(/http:\/\/[^\/]*?\/posted?\.php/.test(document.location.href)){var dialog=new Dialog().setClassName('share_congratulations').setTitle(tx('sh03')).setBody('<div class="status"><h2>'+tx('sh04')+'</h2><p>'
+tx('sh05')+'</p></div>').setButtons(Dialog.OK).show();}else{throw 0;}}
...
new AsyncRequest().setURI('/ajax/share_misc.php').setData({share_id:share_id,comment:comment}).setHandler(function(parentNode,commentsNode,response){hide(parentNode);to_array(DOM.createFromMarkup(response.getPayload())).forEach(commentsNode.appendChild,commentsNode);node.getElementsByTagName('textarea')[0].value='';}.bind(this,node,$('share_comments_for_'+share_id))).send();obj.disabled=true;return true;}
...
new AsyncRequest().setURI('/ajax/share_misc.php').setData({share_id:share_id,delcomment:1,comment_id:comment_id}).send();return true;}.bind(null,obj,share_id,comment_id);var dialog=new Dialog().setTitle(tx('sh06')).setBody(tx('sh07')).setButtons([Dialog.newButton('delete',tx('sh08')),Dialog.CANCEL]).setHandler(delete_function).show();return false;}
function _share_flash_comment_error(obj){animation(obj).from('background','#ffc8c8').to('background','#fff').duration(500).go();}
function share_show_delete_dialog(context,href){var dialog=new contextual_dialog();dialog.set_context(context);dialog.show_choice(tx('sh10'),tx('sh11'),tx('sh12'),function(){_share_delete_clicked(this,context,href);},tx('sh09'),function(){generic_dialog.get_dialog(this).hide();});return false;}
...
function _share_onsubmit_to(ids,post_action,obj){var ids=$(ids);if(!ids.tokenizer||ids.tokenizer.count_names()==0){new ErrorDialog().showError(tx('sh:required'),tx('sh13'));return false;}
...
var ChannelRebuildReasons={Unknown:0,AsyncError:1,TooLong:2,Refresh:3,RefreshDelay:4,UIRestart:5,NeedSeq:6,PrevFailed:7,IFrameLoadGiveUp:8,IFrameLoadRetry:9,IFrameLoadRetryWorked:10};function ChannelManager(user){this.user=user;this.iframeCheckTime=16000;this.iframeCheckRetryTime=16000;this.defaultRetryInterval=3000;this.maxRetryInterval=60000;this.iframeLoadMaxRetries=1;this._init();}
...
function Presence(user,name,firstName,serverTime,inPopoutWindow,sitevars,popoutURL,loginURL){this.user=user;this.name=name;this.firstName=firstName;this.sitevars=sitevars;this.popoutURL=popoutURL;this.loginURL=loginURL;var viewportVector=Vector2.getViewportDimensions();this.maxTabOffset=66;if(ge('intern_bookmark_frame')){this.maxTabOffset+=25;}
if(ge('fbnew_preview_bar')){this.maxTabOffset+=25;}
...
if(obj.type=='app_msg'){Arbiter.inform(Presence.getArbiterMessageType(obj.app_id,obj.event_name),obj.message,Arbiter.BEHAVIOR_PERSISTENT);}
...
(extra?extra:'')+'>'+text+'</a>';},setHolderClass:function(className){if(className=='popped_out'){if(!this.sitevars['PRESENCE_APP_DOCK_IS_LIVE']){CSS.removeClass(this.holder,'full');}}else if(className=='full'){CSS.removeClass(this.holder,'popped_out');}else{CSS.removeClass(this.holder,'popped_out');if(!this.sitevars['PRESENCE_APP_DOCK_IS_LIVE']){CSS.removeClass(this.holder,'full');}}
CSS.addClass(this.holder,className);},_loadHolderClass:function(){if(!this.poppedOut&&!this.inPopoutWindow&&this.holder.className!=''){this.setHolderClass(statusControl.visibility||chatDisplay.numTabs>0?'full':'');}},checkRebuild:function(){if(this.isShutdown){channelManager.rebuild();}},renderLoginLink:function(){return this.renderLink(this.loginURL,'log in');},getErrorDescription:function(asyncResponse){var error=asyncResponse.getError();var desc=asyncResponse.getErrorDescription();if(!desc){desc=tx('sh:error-occurred');}
...
return false;},loginShutdown:function(){var reason=tx('pr02',{'login':this.renderLoginLink()});this.shutdown(false,reason);},connectionShutdown:function(shouldDelay){var reason=tx('pr04',{'Chat':tx('chd01')});this.shutdown(shouldDelay,reason);},maintenanceShutdown:function(){var reason=tx('pr10',{'Chat':tx('chd01')});this.shutdown(false,reason);channelManager.stop();},versionShutdown:function(){var reason=tx('pr08',{'Chat':tx('chd01')});this.shutdown(false,reason);channelManager.stop();},shutdown:function(shouldDelay,reason){this.isRestarting=false;this.isShuttingDown=true;var now=(new Date()).getTime();this.shutdownTime=now;if(!shouldDelay){this._shutdown(reason,0);}else{setTimeout(this._shutdown.bind(this,reason,now),this.shutdownDelay);}},_shutdown:function(reason,shutdownTime){if(!this.isShuttingDown&&shutdownTime==this.shutdownTime){return;}
...
if(!this.inPopoutWindow){CSS.removeClass(this.holder,'presence_error');}else{if(this.shutdownErrorDialog){this.shutdownErrorDialog.hide();}}},start:function(){for(var i=0;i<this.startHandlers.length;i++){this.startHandlers[i]();}},registerResizeHandler:function(fn){this.resizeHandlers.push(fn);},registerStateStorer:function(fn){this.stateStorers.push(fn);},registerStateLoader:function(fn){this.stateLoaders.push(fn);},registerMsgHandler:function(fn){this.msgHandlers.push(fn);},registerShutdownHandler:function(fn){this.shutdownHandlers.push(fn);},registerRestartHandler:function(fn){this.restartHandlers.push(fn);},registerStartHandler:function(fn){this.startHandlers.push(fn);},registerTabOpenHandler:function(fn){this.tabOpenHandlers.push(fn);},registerTabCloseHandler:function(fn){this.tabCloseHandlers.push(fn);},registerTempTabCloseHandler:function(handler){this.tempTabCloseHandler=handler;}};function getDimensions(id){var element=ge(id);var els=element.style;var originalVisibility=els.visibility;var originalPosition=els.position;var originalDisplay=els.display;if(originalDisplay=='none'){els.visibility='hidden';els.position='absolute';els.display='block';}
...
search_typeaheadpro.prototype.show=function(){if(!this.less_than_n_chars){CSS.addClass(this.dropdown,'typeahead_search');this.parent.show();this.dropdown.style.width='150px';}else{this.hide();}}
...
this.parent.found_suggestions(suggestions,text,fake_data);if(this.suggestion_count>0){CSS.addClass(this.list.firstChild.firstChild,'blue_top_border');CSS.removeClass(this.list,'no_border_list');CSS.addClass(this.list.lastChild.lastChild,'blue_bottom_border');}else{CSS.addClass(this.list,'no_border_list');}
...
if(presence.isIE6){dateMarkup+='<br />&nbsp;<br />&nbsp;';}
...
if(this.shiftByNumTabs)this.numShift=this.numToShow;if(this._s!=null)this._setPos(this._s);},_setMaxWidth:function(){var w=document.body.offsetWidth;var divs=['buddy_list_tab','status_control_tab','presence_notifications_tab','presence_applications_tab','icon_garden','bookmarkable_app'];for(var i=0;i<divs.length;i++){w-=(ge(divs[i])&&$(divs[i]).clientWidth!=undefined)?ge(divs[i]).clientWidth:0;}
...
function ChatStatusControl(visibility){this.user=presence.user;this.visibility=visibility;this.poppedOut=presence.poppedOut;this._init();}
...
var classes=new Array('unavailable','available','idle');for(var i=0;i<=classes.length;i++){if(i==availability){CSS.addClass(this.availabilityToggle,classes[i]);if(this.statusControlTab){CSS.addClass(this.statusControlTab,classes[i]);}}else{CSS.removeClass(this.availabilityToggle,classes[i]);if(this.statusControlTab){CSS.removeClass(this.statusControlTab,classes[i]);}}}},_setIdle:function(){this._updateAvailabilityUI(2);},setVisibility:function(visibility){if(visibility==this.visibility){return;}
this.visibility=visibility;if(visibility){this._updateAvailabilityUI(1);channelManager.isActionRequest=true;channelManager.rebuild(ChannelRebuildReasons.UIRestart);}else{this._updateAvailabilityUI(0);}
for(var i=0;i<this.visibilityHandlers.length;i++){this.visibilityHandlers[i]();}},_onVisibilityResponse:function(newVisibility,response){presence.pauseSync();this.setVisibility(newVisibility);CSS.removeClass(this.availabilityToggleImage,'availability_loading');if(!presence.inPopoutWindow&&!newVisibility){chatDisplay.unfocus();}
presence.resumeSync();},_onVisibilityError:function(response){var chat=tx('chd01');presence.showAsyncError(response,tx('chs01',{'Chat':chat}));CSS.removeClass(this.availabilityToggleImage,'availability_loading');},sendVisibility:function(visibility){if(this.visibility==visibility){return;}
CSS.addClass(this.availabilityToggleImage,'availability_loading');this.visibilityAsync=new AsyncRequest().setHandler(this._onVisibilityResponse.bind(this,visibility)).setErrorHandler(this._onVisibilityError.bind(this)).setTransportErrorHandler(this._onVisibilityError.bind(this)).setData({'visibility':visibility}).setURI(chatDisplay.settingsURL).send();},getSetting:function(name){return $('chat_setting_checkbox_'+name).checked?1:0;},setSetting:function(name,value){if(this.getSetting(name)==value){return;}
...
function ChatBuddyListTypeahead(obj){this.curStr='';this.clearDiv=null;this.focused=false;this.selected=null;this.selectedIndex=0;this.selectableCount=0;this.minBuddyCount=10;this.obj=obj;this.obj.typeahead=this;this.placeholderText=obj.value;this.inputDiv=ge('buddy_list_typeahead');this.shouldShowClear=!presence.isSafari2;if(!this.shouldShowClear){obj.onmousedown=this._onmousedown.bind(this);}
...
this.getBuddyItemName(id).innerHTML=nameCapped;},showEmptySearch:function(){show('buddy_list_empty_search');},hideEmptySearch:function(){hide('buddy_list_empty_search');},getSortedList:function(){if(this.sortedList.length==0&&count(this.availableList)==this.availableCount){this.sortedList=this._sort(keys(this.availableList));}
...
elem.innerHTML=this._renderItem(id,flid,isIdle);elem.style.height='0px';DOM.insertAfter(prevElem.parentNode,prevElem,elem);var itemHeight=isIdle?this.compactItemHeight:this.itemHeight;if(shouldAnimate){var anim=animation(elem);if(shouldDelayExpand){anim.duration(this.expandAnimDuration+500).checkpoint();}
...
this._hideBuddyListEmptyItems();var nowAvailableIDs=keys(nowAvailableList);var shouldAnimate=!this.showingError&&!presence.isSafari2&&(wasAvailableIDs.length+nowAvailableIDs.length<this.maxItemsToAnimate);if(this.rendered){for(var i=0;i<wasAvailableIDs.length;i++){if(wasAvailableList[wasAvailableIDs[i]]){this._collapseItem(wasAvailableIDs[i],wasAvailableList[wasAvailableIDs[i]],shouldAnimate,flid);}}
...
set_inner_html(emptyItem,emptyText);}}},_clearSpinningIndicator:function(flids){for(var i=0;i<flids.length;i++){var flid=flids[i];CSS.removeClass($(this._getFriendListId(flid)),'chat_setting_loading');}},_renderFriendListItem:function(flid){var fl_name=this.flData[flid].n;var online=this.flData[flid].o;var markupArr=['<div class="friendlist_status">','<strong>',htmlize(fl_name),'</strong>','</div>'];if(typeof this.flData[flid].s=='undefined'){var onmouseover=sprintf('buddyList._friendListHandleMouseOver(%d);',flid);var onmouseout=sprintf('buddyList._friendListHandleMouseOut(%d);',flid);var onclick=sprintf('buddyList._friendListHandleOnClick(%d);',flid);markupArr.push('<div class="online_checkbox">','<span class="show_loading">','<img src="/images/loaders/indicator_blue_small.gif" />','</span>','<span class="hide_loading">','<input type="checkbox" ',(online?'checked="checked" ':''),'id="',this._getFriendListOnlineCheckboxId(flid),'" ','onmouseover="',onmouseover,'"','onmouseout="',onmouseout,'"','onclick="',onclick,'"',' />','</span>','<div class="titletip">',tx('chb08'),'</div>','</div>');}
return markupArr.join('');},_renderBuddyContent:function(){var markupArr=['<div id="buddy_list_all" class="subgroup">','<div id="buddy_list_empty_search" class="info_text" style="display:none">',tx('chb06'),'</div>','<ul id="buddy_list_parent" class="list_select">'];var flids=[];var groupedLists={};if(this.flMode){flids=keys(this.flData);groupedLists=this._groupAvailableListByFl(true);}else{flids=[null];}
...
markupArr.concat(['</ul>','</div>']);return markupArr.join('');},_render:function(){if(this.sortedList.length==0&&count(this.availableList)==this.availableCount){this.sortedList=this._sort(keys(this.availableList));}
