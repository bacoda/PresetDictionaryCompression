/*       Source:  Local Cache                                                                 */
/*     Location:  rsrc:120076:nu_ll:/html/js/fbml.js                                          */
...
function fbmlLogin(app_id){new AsyncRequest().setURI('/ajax/api/tos.php').setData({app_id:app_id,grant_perm:1,profile_id:typeof PROFILE_OWNER_ID=='undefined'?0:PROFILE_OWNER_ID,api_key:$('api_key').value,auth_token:$('auth_token').value}).setHandler(bind(this,function(response){if(response.getPayload()){form_hidden_inputs=response.getPayload();}
...
function promptPermission(app_id,perms,next_js,close_js){var async=new AsyncRequest().setURI('/fbml/ajax/permissions.php').setData({app_id:app_id,perms:perms});this.grantedPerms=false;new Dialog().setAsync(async).setHandler(function(button){this.grantedPerms=true;Dialog.getCurrent()._submitForm('POST','/fbml/ajax/permissions.php',button);return false;}.bind(this)).setCloseHandler(function(next_js,close_js){if(this.grantedPerms){next_js();}
close_js();}.bind(this,next_js,close_js)).show();}
...
fbjs_main.prototype.setLocation=function(url){url=fbjs_sandbox.safe_string(url);if(fbjs_dom.href_regex.test(url)){document.location.href=url;return this===window?null:this;}else{fbjs_console.error(url+' is not a valid location');}}
...
fbjs_facebook.prototype.isApplicationAdded=function(){return fbjs_private.get(this).sandbox.data.loggedin;}
...
var priv=fbjs_private.get(this);priv.appid=appid;priv.event_name=event_name;priv.send_success_handler=null;priv.send_error_handler=null;var oninform=function oninform(type,data){callback_fn(data);};var type=Presence.getArbiterMessageType(appid,event_name);priv.arbiter=Arbiter.subscribe(type,oninform);}
...
fbjs_livemessage.prototype.setShutdownHandler=function(callback_fn){var priv=fbjs_private.get(this);if(priv.arbiter_shutdown){Arbiter.unsubscribe(priv.arbiter_shutdown);priv.arbiter_shutdown=null;}
priv.arbiter_shutdown=Arbiter.subscribe(Presence.ARBITER_SHUTDOWN,callback_fn.shield());return this;}
fbjs_livemessage.prototype.setRestartHandler=function(callback_fn){var priv=fbjs_private.get(this);if(priv.arbiter_restart){Arbiter.unsubscribe(priv.arbiter_restart);priv.arbiter_restart=null;}
priv.arbiter_restart=Arbiter.subscribe(Presence.ARBITER_RESTART,callback_fn.shield());return this;}
...
return domElement;};
if (window.Bootloader) { Bootloader.done(["js\/fbml.js"]); }