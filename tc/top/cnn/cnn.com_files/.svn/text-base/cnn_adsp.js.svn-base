...
/* Opera removed to avoid oBench frame issus
...
function cnnad_createAd(adId,cnnad_url,cnnad_height,cnnad_width,target) {
	cnnad_url = cnnad_preview(cnnad_url);
	cnnad_url += '&tile=0&domId=' + adId;
	cnnad_createAdHelper
(adId,cnnad_url,cnnad_height,cnnad_width,target,false);
}
...
function cnnad_getDEAdHeadCookie( imageRef ) {
	...
	var newSrc = "cnn.com_files/1.gif";
