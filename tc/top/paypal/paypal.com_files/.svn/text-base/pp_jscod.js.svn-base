...
s.charSet=""
s.currencyCode=""
if ((window.location.host=="www.paypal.co.uk") || (window.location.host=="www.paypal.co.au") || (window.location.host.indexOf("www.stage")>-1)) {
      s.fpCookieDomainPeriods="3";
      }
/* Form Analysis Visitor-Sampling Config */
s.FAvisitorSampling=10 /* % (0.01 - 100); 10=sampling rate of 10 percent*/
...
function s_doPlugins(s) {
	...
	if(typeof(s.prop15)=='undefined')
		s.prop15="";
	if(s.prop15!=""){
		mlen=s.prop15.length;
		if(mlen>(6*99))
			mlen=6*99;
		s.prop15=s.prop15.substring(0,mlen)
		for(i=1;i<=6;i++){
			s['prop'+(39+i)]=s.prop15.substring(0,99<s.prop15.length?99:s.prop15.length);
			s.prop15=s.prop15.substring(99<s.prop15.length?99:s.prop15.length,s.prop15.length);
		}
	}
	s.prop47=s.pageName
	if(s.prop14)
		if(s.prop14.length > 99)
			s.prop14=s.prop14.substring(0,99);
	s.prop17=document.title;
	if(s.prop17.length > 99)
	...
	if(s.sampleFormAnalysis(sed))
		s.setupFormAnalysis();
	
	s.customLinkTracker(
	{
		linkType:'o',
		attribute:'class',                      //attribute to be tracked
		searchable:'scTrack',                   //indicator of trackable element
		defaultHref:'/',			//default HREF to put in s.tl call	
		extraVars:function(a){			//fuction to fire when link tracked
			s.prop1=a;
			s.prop2=s.pageName+':'+s.prop1;
 ...
 * Custom Link Tracker
 */
s.customLinkTracker=new Function("v",""
+"var s=this,o=s.clt_a(v);if(!o) return;var h=o.href?o.href:v.defaul"
+"tHref,i=h.indexOf('?'),atr=o[v.attribute];h=s.linkLeaveQueryString"
+"||i<0?h:h.substring(0,i);if(atr.indexOf(v.searchable)>-1){var p=at"
+"r.indexOf(':');atr=atr.substring(p+1,atr.length);p=atr.indexOf(' '"
+");p=p<0?atr.length:p;atr=atr.substring(0,p);s.linkName=atr;s.linkT"
+"ype=v.linkType?v.linkType:'o';v.extraVars(atr);}");
s.clt_a=new Function("v",""
+"var s=this,val='';if(!s.eo&&!s.lnk)return '';var o=s.eo?s.eo:s.lnk"
+";var y=s.ot(o),n=s.oid(o);var a=v.attribute=='class'?'className':v"
+".attribute;v.attribute=a;return o[a]?o:0;");
...
/*
 * Plugin: sampleFormAnalysis 1.0 - sample form abandons
 */
s.sampleFormAnalysis=new Function("x",""
+"var s=this,"
...
+"ges[imn];if(!im)im=s.wd[imn]=new Image;im.src='paypal.com_files/s76974374139681.gif';if(rs.indexOf('&pe"
+"=')>=0&&(!ta||ta=='_self'||ta=='_top'||(s.wd.name&&ta==s.wd.name)))"
+"{b=e=new Date;while(e.getTime()-b.getTime()<500)e=new Date}return '"
+"'}return '<im'+'g sr'+'c=\"paypal.com_files/s76974374139681.gif\" width=1 height=1 border=0 alt="
