...
jQuery.fn.extend({mapClass:function(map){map['?']=map['?']||[];return this.each(function(){var unique={};var cl=[];$.each($.map(this.className.split(/\s+/),function(k){return k in map?map[k]:('*'in map?map['*']:k)}).concat(map['+']),function(i,k){if(k&&!(k in unique)){unique[k]=true;cl.push(k);}});this.className=(cl.length?cl:map['?']).join(' ');});},setClass:function(c1){return this.each(function(){this.className=c1});},toggleClasses:function(c1,c2,force){var map={'?':force};map[c1]=c2;map[c2]=c1;return this.mapClass(map);}});function split_if_string(list,sep){try{return list.length?list.split(sep||/\s+/):[]}catch(e){return list||[]}}
function join_wrap(a,elem_prefix,elem_suffix,list_prefix,list_suffix){var result='';a=split_if_string(a);if(a&&a.length){var ep=elem_prefix||'';var es=elem_suffix||'';result=(list_prefix||'')+ep
+a.join(es+ep)
+es+(list_suffix||'');}
return result;}
function map_list_to_set(list,map_fn){list=split_if_string(list);if(!list||!list.length)
return;if(map_fn===undefined)
...
update_tags:function(tags,how,annotate){var update_map=this.map_tags(tags=split_if_string(tags));var new_tags=$.map(tags,function(t){var bt=bare_tag(t);if(bt in update_map)
...
var base_classes='tag '+(annotate?annotate+' ':'');$changed_tags.each(function(){this.className=$.trim(base_classes+tag_style($(this).text()));});return this},remove_tags:function(tags){if(!tags||tags.length)
tags=this.map_tags(tags);$.each(tags,function(bt,entry){$(entry).parents('li').eq(0).remove()});return this},set_tags:function(tags){var allowed_tags=map_list_to_set(tags=split_if_string(tags),bare_tag);this.remove_tags(this.map_tags(function(bt){return!(bt in allowed_tags)}));return this.update_tags(tags,'append')},fetch_tags:function(){var bar=this;$.post('/ajax.pl',{op:this.tagbar_data.fetch_op,id:this.tagbar_data.item_id,no_markup:1},function(response){bar.set_tags(response)});return this},click_tag:function(tag,op){if(op=="x")
this.remove_tags(tag);else if(op.length==1&&op==tag[0])
this.update_tags(tag.slice(1));else if(op!=tag)
this.update_tags(op+tag);return this}};var twidget_fns={init:function(firehose_id){this.tagwidget_data={item_id:firehose_id}
$(this).append(create_tag_bar(firehose_id,'top'))
.append(create_tag_bar(firehose_id,'user'))
.append(create_tag_bar(firehose_id,'system'));return this},each_bar:function(fn){$('.tbar',this).each(fn);return this},set_tags:function(tags){var widget=this;$.each(tags.split('\n'),function(){var which_bar='user';var this_bars_tags=this;var match=/^<(\w+)>?(.*)$/.exec(this);if(match){which_bar=match[1];this_bars_tags=match[2];}
$('.tbars .tbar.'+which_bar,widget).each(function(){this.set_tags(this_bars_tags)})});return this},fetch_tags:function(){var widget=this;$.post('/ajax.pl',{op:'tags_get_combined_firehose',id:this.tagwidget_data.item_id,no_markup:1},function(response){widget.set_tags(response)});return this},open:function(){$(this).show()
.find(':text')
.each(function(){this.focus()});return this},close:function(){$(this).hide();return this}};function open_tag_widget(event,selector){$(selector||this).parents('[id^=firehose-]').andSelf()
.find('.tag-widget:hidden')
.filter('.stub').each(function(){$.extend(this,twidget_fns);this.init(firehose_id_of(this.id))}).removeClass('stub')
.end().each(function(){this.open();this.fetch_tags();});}
function close_tag_widget(event,selector){$(selector||this).parents('[id^=firehose-]').andSelf()
.find('.tag-widget:visible')
.each(function(){this.close()})}
function create_tag_bar(item_id,bar_kind,tags){bar_kind=bar_kind in create_tag_bar.bar_templates?bar_kind:'user';var tmpl=create_tag_bar.bar_templates[bar_kind];var new_bar=$.extend($('<div class="'+bar_kind+' tbar"><ul></ul></div>')[0],tbar_fns,{tagbar_data:{fetch_op:tmpl.fetch_op,item_id:item_id,menu_template:join_wrap(tmpl.menu_cmds,'<li>','</li>','<ul class="tmenu">','</ul>')}});new_bar.tagbar_data.list_el=$('ul',new_bar)[0];if(tags!==undefined)
new_bar.update_tags(tags);return new_bar;}
create_tag_bar.bar_templates={user:{fetch_op:'tags_get_user_firehose',menu_cmds:'! x'},top:{fetch_op:'tags_get_top_firehose',menu_cmds:'_ # ! )'},system:{fetch_op:'tags_get_system_firehose',menu_cmds:null}}
...
function firehose_id_of(expr){try{if(typeof expr==='number')
return expr;else if(typeof expr==='object'&&expr.parentNode){if(expr.id&&expr.id.match(/-\d+$/))
expr=expr.id;else
expr=$(expr).parents('[id^=firehose-]').attr('id');}
var match=/(?:.+-)?(\d+)$/.exec(expr);if(match)
...
if(name=="setfhfilter"){firehose_settings.fhfilter=value;firehose_settings.page=0;firehose_settings.more_num=0;}
...
function ajax_update(request_params,id,handlers,options){if(!options)
options={};var opts={url:options['request_url']||'/ajax.pl',data:request_params,type:'POST',contentType:'application/x-www-form-urlencoded'};if(options['async_off'])
opts['async']=false;if(id){opts['success']=function(html){$('#'+id).html(html);}}
...
myAnim.onComplete.subscribe(function(){var fh_node=$dom(fh);if(fh_node){fh_node.style.height="";if(fh_use_jquery){$("h3 a[class!='skin']",fh_node).click(function(){var h3=$(this).parent('h3');h3.next('div.hid').and(h3.find('a img')).toggle("fast");return false;});}}});if(need_animate){myAnim.animate();}}else if(el[0]=="remove"){var fh_node=$dom(fh);if(fh_is_admin&&fh_view_mode=="fulltitle"&&fh_node&&fh_node.className=="article"){}else{var attributes={height:{to:0}};if(!is_ie){attributes.opacity={to:0};}
