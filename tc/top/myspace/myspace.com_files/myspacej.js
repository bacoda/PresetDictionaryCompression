...
function generalizeDomain(){
	/* Opera removed to avoid frame load issues in oBench
...
MySpace.UI.HeaderStatusMood.prototype={
	initialize:function(){
		...
		if(this._statusNode.childNodes.length > 0){
			...
			this._saveStatusInCookie({
				...
				moodValue: this._currentMood,
				moodName: this._getText(this._moodNameNode),
				moodPictureURL: this._moodImage.src
			});
		}
	...
	_onListsError:function(){
		var opt=new Option(this._getText(this._moodNameNode), this._currentMood);
		this._moodSelect.options[0]=opt;
	},
	
	_onClickStatusChoice:function(e){
		this._cancelEvent(e);
		this._statusInput.value=this._getText(e.target);
	...
	output.toggleInternationalLinks = function()
	{
		...
		else
		{
			...
			internationalLinks.style.top = 
				(- internationalLinks.offsetHeight) + 'px';
...
//TODO: Clean up Fader code below
...
function colorFade(target,start,end,steps,speed) {
  ...
  if(bint == 0) { bint = 1 }
  target.step = 1;
  target.timer = setInterval( function() { animateColor(target,steps,er,eg,eb,rint,gint,bint) }, speed);
}
...
function fader(endColor, steps){
	
	var el = document.getElementsByTagName("BODY")[0];
	
    if (Sys.Browser.agent === Sys.Browser.InternetExplorer) {
        el.style.backgroundColor = endColor;
        return;
    }
	...
	if(document.defaultView){
		startColor = document.defaultView.getComputedStyle(el,'').getPropertyValue('background-color');
		startColor = startColor.replace('rgb(','').replace(')','').split(',');
	}
	else{
		startColor = el.currentStyle["backgroundColor"].replace('#','');
	}

	colorFade(el, startColor, endColor.replace('#',''), steps, 20);
}